...
//----------------------------------------------------------------------
// OpenFile::AppendSize
//  return openfile need open AppendSize or not
//----------------------------------------------------------------------

bool
OpenFile::AppendSize(int numBytes)
{
    //calculate sectors be needed and fetch the bitmap
    OpenFile *newFile = new OpenFile(0);        //第0块指向存放bitmap的扇区
    BitMap *freeMap = new BitMap(NumSectors);   //bitmap
    freeMap->FetchFrom(newFile);
    //ask for new space
    if(!hdr->AppendSector(freeMap,numBytes))
        return false;
    //write back the bitmap
    freeMap->WriteBack(newFile);
    delete newFile;
    delete freeMap;
    return true;
}	
//----------------------------------------------------------------------
// OpenFile::WriteBack
//  extension file open write back sector into file
//	
//----------------------------------------------------------------------
...
{
    OpenFile *file = new OpenFile(1);   //第一块指向存放filehdr索引目录的sector
    Directory *directory = new Directory(10);   //directory里面有10块sector
    directory->FetchFrom(file);
    //write back sector into file
    int sector = directory->Find(name);
    hdr->WriteBack(sector);
    //write back the hdr:remends the numbytes,numsector datasector
    delete file;
    delete directory;
}	
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
